...
idx2utt, utt2idx, idx2emo, emo2idx, idx2speaker,\
    speaker2idx, weight_matrix, _, _, _, _, _, _, _ = load_erc()

weight_matrix = weight_matrix.to(device)

_, _, _, _, _, _, _, my_dataset_train, my_dataset_test,\
...
def train(ft_model, trig_model, emo_model, train_data_loader, epochs):
    ...
    for epoch in tqdm.tqdm(range(epochs)):
        ...
        for i_batch, sample_batched in tqdm.tqdm(enumerate(train_data_loader)):
            ...
            emo_dialogue_ids = [d[0].item() for d in sample_batched[0]]
            inputs = sample_batched[1].to(device)
            targets2 = sample_batched[3].to(device)
  
            optimizer.zero_grad()
            
            trig_rep, _ = trig_model(inputs,dialogue_ids,utt_len)
            with torch.no_grad():
                emo_rep, _ = emo_model(emo_dialogue_ids, global_speaker_info, speaker_dialogues, speaker_emotions, speaker_indices, inputs)

            combine_ip = torch.cat([trig_rep,emo_rep],-1)            
            outputs = ft_model(combine_ip)
    ...
    return trig_model, ft_model

def validate(ft_model, trig_model, emo_model, test_data_loader,write_file,epoch,history2_2,canvas2_2,log_step=2):
    print("\n\n***VALIDATION ({})***\n\n".format(epoch))
    ...
    with torch.no_grad():
      ...
      for i_batch, sample_batched in tqdm.tqdm(enumerate(test_data_loader)):
            ...
            trig_rep, _ = trig_model(inputs,dialogue_ids,utt_len_test)
            emo_rep, _ = emo_model(emo_dialogue_ids, global_speaker_info_test, speaker_dialogues_test, speaker_emotions_test, speaker_indices_test, inputs)
...
model = cascade(hidden_size, 2).to(device)

weights2 = [1.0, 2.5]
data_iter_train, data_iter_test = get_train_test_loader(batch_size)
_,__,___ = train(model, trigger_model, emotion_model, data_iter_train, epochs = 1000)
